...
(Debemos considerar la sección 4 de [1] para definir las herramientas que usaremos, trabajaremos sobre un círculo, los términos como $\textit{fase}$ necesitan de ese contexto)

Inicalmente consideremos un oscilador, es decir el destello de una sola luciérnaga, mediante el modelo que se propone en la sección 4.5 de [1]. 
...
Si suponemos que la luciérnaga está aislada, que no hay estimulos para el destello, podemos considerar que la frecuencia con la que se da el destello está dada por $\dot{\theta} = \omega$.

Para añadir un estimulo que altere la fase del destello, considere un estimulo periódico cuya fase $\Theta$ satisface $\dot{\Theta} = \Omega$. 

(Describimos cómo llegar la ecuación $\dot{\theta} = \omega + A sin(\Theta-\theta)$)
"""
