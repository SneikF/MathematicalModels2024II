...
# ╔═╡ 62a7cdc6-f930-11ef-3921-df52533f673d
	using DifferentialEquations , Plots, Random, IJulia, Base64, Statistics, PlutoUI
...
@bind params PlutoUI.combine() do Child
	...
	Shared K:
	$(Child(CheckBox(default=false)))

	K:
	$(Child(Slider(0:0.1:2, default=0.3)))

	Span:
	$(Child(Slider(2:12, default=0.3)))
...
begin
	...
	html *= "<br><span style='font-size:40px;'> Span: [0,  $(params[4])π] </span>"
	HTML(html)
end

# ╔═╡ 59c2af93-5a55-468a-84e1-044471b83302
function setKuramoto(ω, N, sharedK=true)
	function kuramoto_shared!(du, θ, k, t)
		for i in 1:N
			du[i] = ω[i] + k/N * sum(sin(θ[j] - θ[i]) for j in 1:N)
		end
	end
	
	if sharedK
		return kuramoto_shared!
	end
	
	function kuramoto!(du, θ, k, t)
		for i in 1:N
			du[i] = ω[i] + 1/N * sum(k[j, i] * sin(θ[j] - θ[i]) for j in 1:N)
		end
	end
...
function solveKuramoto(N, ω, θ_0, k, span, sharedK, name)
	...
	tspan = (0.0, span * π)
	sol = solve(ODEProblem(kuramoto!, θ_0, tspan, k), saveat=0.06)
	
	colors = [rand(RGB) for _ in 1:N]
	...
	"""
	IJulia.HTML(html_video)
end

# ╔═╡ b46ba03e-fb10-4d04-956d-a2f210abe5fa
begin
	Random.seed!(1234)
	
	N = params[1]
	sharedK = params[2]
	sharedKValue = params[3]
	span = params[4]
	...
	solveKuramoto(N, ω, θ_0, k, span, sharedK, "kuramoto1.mp4")
end
